...
// ExpX with turn ordering checks everytime the turn is ended
public class PlayerStrategyExpectimaxXI extends Player {
  ...
  public PlayerStrategyExpectimaxXI(int number) {

    super(number);
    super.name = "Expectimax XI";
  ...
  public PlayerStrategyExpectimaxXI(int number, int flag) {
  ...
  public void switchOutAsActionScores(Tree treeBuilt, GameState currentGS, ArrayList<Action> possibleActions, int printMode, Boolean average) {
    ...
    for (int i = 0; i < possibleActions.size(); i++) {
      ...
      bestOpIndexes = evaluateExpectimaxTree(treeBuilt, (speedResult == this.opponentPlayerNumber), true);
  ...
  public Action chooseAction(GameState currentGS, ArrayList<Action> possibleActions, int printMode) {
    ...
    this.switchOutAsActionScores(expectimaxTree, currentGSCopy, possibleActions, printMode, true);
  ...
  public float expectimax(TreeNode node, boolean maxingPlayer, float bestForMax, float bestForMin, int maxDepth, Boolean reScore) {
    ...
    if (maxingPlayer) {
      ...
      for (int i = 0; i < numberOfChildren; i++) {
        ...
        expectimax(child, (node.player == child.player), bestForMax, bestForMin, maxDepth, reScore);
