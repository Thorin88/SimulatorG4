...
// Uses dynamic turn ordering
// Gives scores for switches but does not use them in battle
// Has two types of switch considerations:
// - WC assumes opponent predicts the switch
// - Normal assumes the opponent will not expect a switch
public class PlayerStrategyExpectimaxVIII extends Player {
  ...
  public PlayerStrategyExpectimaxVIII(int number) {

    super(number);
    super.name = "Expectimax VIII";
  ...
  public PlayerStrategyExpectimaxVIII(int number, int flag) {
  ...
  public void switchOutAsActionScoresWC(GameState currentGS, ArrayList<Action> possibleActions, int printMode) {
    ...
    for (int i = 0; i < possibleActions.size(); i++) {
      ...
      finalScore = expectimax(root,false,MIN,MAX,4,true);
  ...
  // Assumes that currentGS is already loaded with AI data if applicable
  public void switchOutAsActionScores(Tree treeBuilt, GameState currentGS, ArrayList<Action> possibleActions, int printMode) {
    ...
    for (int i = 0; i < possibleActions.size(); i++) {
      ...
      if (speedResult == this.opponentPlayerNumber) {
        ...
        bestOpIndexes = evaluateExpectimaxTree(treeBuilt, true);

      }
      // Could be combined but left separate to show it could be different
      else {

        bestOpIndexes = evaluateExpectimaxTree(treeBuilt, false);
  ...
  public Action chooseAction(GameState currentGS, ArrayList<Action> possibleActions, int printMode) {
    ...
    this.switchOutAsActionScores(expectimaxTree, currentGSCopy, possibleActions, printMode);
