...
public class PlayerStrategyMinimax_RangesII extends Player {
  ...
  public PlayerStrategyMinimax_RangesII(int number) {
  ...
  public PlayerStrategyMinimax_RangesII(int number, int flag) {
  ...
  public Action chooseAction(GameState currentGS, ArrayList<Action> possibleActions, int printMode) {
    ...
    // if (true) {
      ...
      // Builds first set of moves
      minimax(minimaxTree.root,true,MIN,MAX,1);

      ArrayList<TreeNode> rootChildren = minimaxTree.root.children;
      int size = minimaxTree.root.children.size();

      float bestSoFar = MIN;

      for (int i = 0; i < size; i++) {

        // Starting from opponents move
        float moveScore = minimax(rootChildren.get(i),false,MIN,MAX,4);

        // Note these can all == MIN
        if (moveScore >= bestSoFar) {
          ...
          bestSoFar = moveScore;
          bestMoveIndexes.add(rootChildren.get(i).moveIndex);
      ...
      if (printMode == PRINTING_ENABLED) {
        ...
        for (int i = 0; i < bestMoveIndexes.size(); i++) {

          System.out.println(bestMoveIndexes.get(i));
    ...
    //
    //   // Builds first set of moves
    //   float minRootValue minimax(minimaxTree.root,false,MIN,MAX,1);
    //
    //   ArrayList<TreeNode> rootChildren = minimaxTree.root.children;
    //   int size = minimaxTree.root.children.size();
    //
    //   float bestSoFar = MAX;
    //   ArrayList<Integer> bestOppMoveIndexes = new ArrayList<Integer>();
    //
    //   for (int i = 0; i < size; i++) {
    //
    //     // Starting from opponents move
    //     float moveScore = minimax(rootChildren.get(i),true,MIN,MAX,4);
    //
    //     // Note these can all == MIN
    //     if (moveScore <= bestSoFar) {
    //
    //       if (moveScore < bestSoFar) {
    //         bestOppMoveIndexes.clear();
    //       }
    //
    //       bestSoFar = moveScore;
    //       bestOppMoveIndexes.add(rootChildren.get(i).moveIndex);
    //
    //     }
    //
    //   }
    //
    //   int size = bestOppMoveIndexes.size();
    //
    //   float bestSoFarForUs = MIN;
    //
    //   for (int i = 0; i < size; i++) {
    //
    //     // Starting from opponents move
    ...
    //
    //     // Note these can all == MIN
    //     if (moveScore <= bestSoFarForUs) {
    //
    //       if (moveScore < bestSoFarForUs) {
    //         bestOppMoveIndexes.clear();
    //       }
    //
    //       bestSoFarForUs = moveScore;
    //       bestOppMoveIndexes.add(rootChildren.get(i).moveIndex);
    //
    //     }
    //
    ...
    // }
      ...
      bestMoveIndexes = this.evaluateMinimaxTree(minimaxTree, false);
      ...
      if (printMode == PRINTING_ENABLED) {
        ...
        // System.out.println("Minimax score: " + minimax(testTree.root,true,MIN,MAX,4) );
        System.out.println("Minimax root score: " + minimaxTree.root.currentScore );
        System.out.println("Minimax best move indexes: " + evaluateMinimaxTree(minimaxTree, false).toString() );
    ...
    if (bestMoveIndexes.size() != 0) {

      indexOfAction = bestMoveIndexes.get(UtilityFunctions.randomNumber(0,bestMoveIndexes.size() - 1));
  ...
  public float minimax(TreeNode node, boolean maxingPlayer, float bestForMax, float bestForMin, int maxDepth) {
    ...
    if (maxingPlayer) {
      ...
      for (int i = 0; i < numberOfChildren; i++) {
        ...
        // done for
        // if (bestForMin <= bestForMax) { System.out.println("(Max) i: " + i + ", depth: " + node.depth); break; }
