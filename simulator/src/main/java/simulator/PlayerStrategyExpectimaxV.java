...
public class PlayerStrategyExpectimaxV extends Player {
  ...
  public PlayerStrategyExpectimaxV(int number) {

    super(number);
    super.name = "Expectimax V";
  ...
  public PlayerStrategyExpectimaxV(int number, int flag) {
  ...
  public Action chooseAction(GameState currentGS, ArrayList<Action> possibleActions, int printMode) {
    ...
    if (this.dataAccessFlag != FULLY_OBSERVABLE && this.opponentTeam.size() == 0) {
    ...
    if (this.dataAccessFlag != FULLY_OBSERVABLE && !this.opponentTeam.contains(opponentPokemon)) {
    ...
    if (this.dataAccessFlag != FULLY_OBSERVABLE) {
      ...
      int numOfOpPmanSeen = this.opponentTeam.size();
      for (int i = 0; i < numOfOpPmanSeen; i++) {

        Pokemon tempPman = this.opponentTeam.get(i);
  ...
  public void moveSeen(GameState currentGS, Pokemon user, Pokemon target, int moveIndex, Estimates estimates, int printMode) {
    ...
    int indexOfUser = this.opponentTeam.indexOf(user);
    ...
    // Update their defense stats
      ...
      int indexOfTarget = this.opponentTeam.indexOf(target);
